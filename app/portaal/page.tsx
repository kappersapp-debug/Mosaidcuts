...
function DashboardView() {
  ...
  useEffect(()=>{
    ...
    fetch('/api/portaal/bookings?filter=upcoming').then(r=>r.json()).then(d=>setUpcoming((d.bookings??[]).slice(0,5)))
    ...
    <div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
          ...
          {/* Legend */}
          <div className="flex flex-wrap gap-3 mt-4 pt-3 border-t border-gray-100 text-xs font-semibold text-gray-500">
          ...
          {filters.map(f=>(
            <button key={f.id} onClick={()=>setFilter(f.id)}
              className={['px-3 py-1.5 rounded-lg text-xs font-bold transition-colors',
                filter===f.id?'bg-white text-brand shadow-sm':'text-gray-500 hover:text-gray-700'].join(' ')}>
              {f.label}
            </button>
          ))}
        </div>
              ...
              <div key={b.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2 flex-wrap">
                    <div className="min-w-0">
                      <p className="font-black text-gray-900 truncate">{b.name}</p>
                      <p className="text-sm text-gray-500 truncate">{b.service} · {formatMedDate(b.date)} · {b.time}</p>
        ...
        {services.map(s=>(
          <div key={s.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-gray-500">{s.desc} · {s.duration} min</p>
            </div>
            <div className="text-right shrink-0">
